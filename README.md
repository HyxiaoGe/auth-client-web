...
`navigator.locks` 和 `BroadcastChannel` 是可选增强：支持时用于跨标签页串行 refresh/reconcile 和即时会话通知；不支持时 SDK 会分别回退到直接请求与 storage 事件协调。
...
|---|---|---|
| `0.3.x`（待发布） | `0.2.x` 全部能力；账户对账还需要 credentialed `/auth/session/reconcile`、带 `state/redirect_uri` 的 credentialed 换票，以及服务端 sid 级撤销与二次 Cookie 会话校验 | 接入多应用账户无感切换前，必须先部署配套 auth-service 与资源服务 sid 校验；不能只升级前端 SDK |
